...
function very_pretty_print(s, bs, bbs)
    pretty = pretty_print(s, bs, bbs)
    very_pretty = pretty[1:2]
    push!(very_pretty, pretty[3][1:4])
    push!(very_pretty[end], (pretty[3][9:16] .== 1)...)
    push!(very_pretty[end], pretty[3][17:18]...)
    push!(very_pretty, pretty[3][5:8])
    push!(very_pretty[end], (pretty[3][9:16] .== 2)...)
    push!(very_pretty[end], pretty[3][19:20]...)
    return very_pretty
end
...
function draw_board_heatmap_2(p, sp; other=false)
    board = zeros(8, 3)
    board[5:6, 3] .= NaN
    board[1:8, 2] = p[6:13]
    if other
        board[5:6, 1] .= NaN
        board[1:4, 1] .= NaN
        board[1:4, 3] = reverse(p[2:5])
        board[7:8, 1] .= NaN
        board[7:8, 3] = reverse(p[14:15])
    else
        board[5, 1] = p[1]
        board[6, 1] = NaN
        board[1:4, 1] = reverse(p[2:5])
        board[1:4, 3] .= NaN
        board[7:8, 1] = reverse(p[14:15])
        board[7:8, 3] .= NaN
    end
    heatmap!(board, sp=sp, c=:thermal)
    vline!(0.5:3.5, sp=sp, c=:black)
    hline!(0.5:4.5, sp=sp, c=:black)
    hline!(6.5:8.5, sp=sp, c=:black)
    annotate!(1, 4.5, sp=sp, text("↑", :green, :center, 8))
    annotate!(1.5, 1, sp=sp, text("→", :green, :center, 8))
    annotate!(1.5, 8, sp=sp, text("←", :green, :center, 8))
    annotate!(1, 6.5, sp=sp, text("↑", :green, :center, 8))
    annotate!(1, 1, sp=sp, text("X", :white, :center, 17, "Helvetica Bold"))
    annotate!(1, 1, sp=sp, text("X", :black, :center, 14))
    annotate!(3, 1, sp=sp, text("X", :black, :center, 14))
    annotate!(2, 4, sp=sp, text("X", :white, :center, 17, "Helvetica Bold"))
    annotate!(2, 4, sp=sp, text("X", :black, :center, 14))
    annotate!(1, 7, sp=sp, text("X", :white, :center, 17, "Helvetica Bold"))
    annotate!(1, 7, sp=sp, text("X", :black, :center, 14))
    annotate!(3, 7, sp=sp, text("X", :black, :center, 14))
    annotate!(2, 5.35, sp=sp, text("______", :black, :center, 8))
    plot!([0.5, 1.5], [4.5, 4.5], sp=sp, fillrange=[5.5, 5.5], fillstyle = :/, color=:white, axis=([], false), ylim=(0.5, 8.5), xlim=(0.5, 3.5))
end

#draw_board_heatmap_2(stats[:, 2, i] ./ total)
