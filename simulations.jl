include("game_half.jl")
using ProgressBars
using StatsBase

function simulate_game(π₁, π₂, s_start, bs, bbs; max_iter=100000, visited=nothing, dark_turns=nothing, rolls=nothing, moves=nothing)
    ...
    for i in 1:max_iter
        roll = wsample(Ps) - 1
        if current_dark
            s, a = π₂(possible_neighs_moves(s, roll, bs, bbs))
        else
            s, a = π₁(possible_neighs_moves(s, roll, bs, bbs))
        end
        if check_bit(s, 32)
